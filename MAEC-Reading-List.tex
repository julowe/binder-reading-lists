...
% Not needed for XeLaTeX (nor do these two lines work...)
...
%function - \printPreceptHeader: checks variable to see if we should print a header with just session, or also with date - then runs appropriate function
\newcommand{\printPreceptHeader}[1]{	
	% check if we should print dates or not
	\ifnum\value{printSessionDate}=0% nope!
		\printHeaderWithSession%
	\else% yep!
		\printHeaderForNextPreceptClassDate{#1}
	\fi
}
...
\fancyhead[R]{%
\Huge\textbf{\textcolor{white}{Eastern Classics~~\\\nouppercase{\leftmark}}}
\vspace{26mm}
}
...
% date dancing, don't change - this gives us the day befor ethe first day of classes
...
\DTMsavedate{lastPrintedClassDate}{\DTMfetchyear{fallStart}-\DTMfetchmonth{fallStart}-\numexpr\DTMfetchday{fallStart}-1}
...
%TODO: give example? Mozi vs Mo Tzu? or too much detail right away?
...
\begin{multicols}{2}
	...
	\printSeminarHeader{lastPrintedClassDate}
	% FIXME: what is this from? book? EC Manual?
	``Kena Upaniṣad'' and ``Mundaka Upanisad''

	\printSeminarHeader{lastPrintedClassDate}
	%FIXME: don't italicize?
	``The Nyāya Sūtra'' in \emph{A Sourcebook in Indian Philosophy}, edited by Sarvepalli Radhakrishnan and Charles A. Moore pp. 358--379.24.

	\printSeminarHeader{lastPrintedClassDate}
	%FIXME: don't italicize?
	``The Vaiśeṣika Sūtra'' and ``The Padārthadharma\-saṁgraha'' in \emph{A Sourcebook in Indian Philosophy}, pp. 387--423.

	\printSeminarHeader{lastPrintedClassDate}
	Tattva-Kaumudî, Kārikās 1--29. Including commentary by Vacaspati Misra.
	\emph{Available in EC Manual}

	\printSeminarHeader{lastPrintedClassDate}
	Tattva-Kaumudî, Kārikās 30--End.
...
% date dancing, don't change - this gives us the day befor ethe first day of classes
%WHY?? well then all the function calls to print the session header below can be the same... confusing? a little, but hopefully void future errors in user-editable section below.
\DTMsavedate{lastPrintedClassDate}{\DTMfetchyear{springStart}-\DTMfetchmonth{springStart}-\numexpr\DTMfetchday{springStart}-1}
% End - 'don't change' section
...
\begin{multicols}{2}
	...
	\printSeminarHeader{lastPrintedClassDate}
	Kālidāsa, \emph{Kumārasaṃbhava}, in \emph{The Origin of the Young God}, translated by Hank Hifetz.
	\emph{Available in EC Manual}
	% TODO: check m dot under in text

	\printSeminarHeader{lastPrintedClassDate}
	Kālidāsa, \emph{The Recognition of Śhakuntalā}, translated by W.J. Johnson.

	\printSeminarHeader{lastPrintedClassDate}
	``The \emph{Dhvanyāloka} of Ānandavardhana with the \emph{Locana} of Abhinavagupta'' selections with supplemental material by Keith and Perry. Through pp. 119.
	\emph{Available in EC Manual}

	\printSeminarHeader{lastPrintedClassDate}
	``The \emph{Dhvanyāloka} of Ānandavardhana with the \emph{Locana} of Abhinavagupta'' selections with supplemental material by Keith and Perry. Sections through pp. 696.
	\emph{Available in EC Manual}
	...
	\printSeminarHeader{lastPrintedClassDate}
	``Discourse to Prince Abhaya,'' ``Discourse to Poṭṭha\-pāda,'' ``Discourse on the Threefold Knowledge'' from \emph{Early Buddhist Discourses}.

	\printSeminarHeader{lastPrintedClassDate}
	``Discourse to Assalāyana'' and ``Discourse to the Layman Sigāla'' from \emph{Early Buddhist Discourses. Samannaphala Sutta}, (``Fruits of the Homeless Life'').
	\emph{Available in EC Manual}.
	...
	\noindent Translation options:

	(A)\emph{Mūlamadhyamakakārikā of Nāgārjuna: The Fundamental Wisdom of the Middle Way}, translated by Jay L. Garfield OR

	(B)\emph{The Philosophy of the Middle Way} translation by David J. Kalupahana.
	%TODO: update to include the Siderits translation? remove kalupahana?
	...
	\printSeminarHeader{lastPrintedClassDate}
	%TODO: quote V. Sutra? chapter title?
	...
	\printSeminarHeader{lastPrintedClassDate}
	Gauḍapāda, ``\emph{Māndukya} Upanishad with the Kārikā of Gauḍapāda'' in Vol. II of \emph{The Upanishads} translated by Swami Nikhilananda (Ramakrishna-Vivekananda Center), pp. 223--368.

	\textbf{Note}: Read both the Māndukya Upanishad and Gauḍapāda commentary. Ignore other commentaries.
	...
	% TODO: check spelling of karika and Gaudapada in actual reading.
	...
	\textbf{Note}: In reading Sankara and Rāmānujā pay close attention to whether you are hearing their own (Siddhanta) positions or their opponents' (Purvapaksha) arguments. The latter can be lengthy and detailed (and plausible).

	\printSeminarHeader{lastPrintedClassDate}
	%TODO: make name same above below, mention EC Manual again?
	Saṅkarâkârya's commentary on the Vedânta Sūtras, pp. 183--191, 199--216, 283--289, 299--308, 312--320, 355--362.
	\emph{Available in EC Manual}

	\printSeminarHeader{lastPrintedClassDate}
	Rāmānujā's commentary on the Vedânta Sūtras, pp. 3--73, \emph{Sacred Books of the East, Vol. 48}, translated by George Thibaut.
	\emph{Available in EC Manual}
	...
	``The \emph{Heart Sūtra''}, pp. 17--53.
	\emph{Available in EC Manual}

	\printSeminarHeader{lastPrintedClassDate}
	Hui-neng, ``Commentary on the Diamond Sūtra'' in \emph{The Sutra of Hui-neng, Grand Master of Zen: With Hui-neng's Commentary on the Diamond Sutra}, translated by Thomas Cleary (Shambhala), pp. 85--144.

	\printSeminarHeader{lastPrintedClassDate}
	Hui-neng, \emph{The Platform Sutra of the Sixth Patriarch}, translated by Philip Yampolsky (Columbia University Press), paragraphs 1--30, (pp. 125--151).

	\printSeminarHeader{lastPrintedClassDate}
	Hui-neng, \emph{The Platform Sutra of the Sixth Patriarch}, translated by Philip Yampolsky (Columbia University Press), paragraphs 31--57, (pp. 151--183).

	\printSeminarHeader{lastPrintedClassDate}
	%TODO: check if correct emph
	``The Great Learning'' and ``The Doctrine of the Mean'' in \emph{A Sourcebook in Chinese Philosophy}, translated and edited by Wing-tsit Chan, pp. 84--114.

	\printSeminarHeader{lastPrintedClassDate}
	Zhu Xi (Chu Hsi), ``The Complete Works of Chu Hsi'' in \emph{A Sourcebook in Chinese Philosophy}, pp. 605--632 (stop at \emph{Jen}).

	\printSeminarHeader{lastPrintedClassDate}
	Zhu Xi (Chu Hsi), ``The Complete Works of Chu Hsi'' in \emph{A Sourcebook in Chinese Philosophy}, pp. 632--653, 693--604.

	\columnbreak% %this is an unfortunate neccessity, otherwise the last few numbers of the previous section get pushed over into the next column by themselves
...
% date dancing, don't change - this gives us the day befor ethe first day of classes
%WHY?? well then all the function calls to print the session header below can be the same... confusing? a little, but hopefully void future errors in user-editable section below.
\DTMsavedate{lastPrintedClassDate}{\DTMfetchyear{summerStart}-\DTMfetchmonth{summerStart}-\numexpr\DTMfetchday{summerStart}-1}
% End - 'don't change' section
...
\begin{multicols}{2}
	...
	\printSeminarHeader{lastPrintedClassDate}
	Kūkai \emph{Major Works} translated by Yoshito S. Hakeda (Columbia University Press), ``The Difference Between Exoteric and Esoteric Buddhism,'' ``Attaining Enlightenment in This Very Existence,'' ``The Meanings of Sound, Word, and Reality''
	\emph{Available in EC Manual}
	...
	\printSeminarHeader{lastPrintedClassDate}
	Dōgen, ``Bendōwa'' in \emph{The Heart of Dōgen's Shōbō\-genzō}, translated by Waddell and Abe (SUNY Press).
	...
	\printSeminarHeader{lastPrintedClassDate}
	Dōgen, ``Genjōkōan'' in \emph{The Heart of Dōgen's Shōbō\-genzō}
	...
	\printSeminarHeader{lastPrintedClassDate}
	Bashō, ``Journey of Bleached Bones in a Field,'' ``Kashima Journal,'' ``Knapsack Notebook'', ``Sarashina Journal'' in \emph{Bashō's Journey: The Literary Prose of Matsuo Bashō}, translated by David Landis Barnhill (SUNY Press)

	\printSeminarHeader{lastPrintedClassDate}
	Bashō, ``The Narrow Road to the Deep North'' in \emph{Bashō's Journey: The Literary Prose of Matsuo Bashō}, translated by David Landis Barnhill (SUNY Press)
...
% date dancing, don't change - this gives us the day befor ethe first day of classes
%WHY?? well then all the function calls to print the session header below can be the same... confusing? a little, but hopefully void future errors in user-editable section below.
\DTMsavedate{lastPrintedClassDate}{\DTMfetchyear{fallPrecept1Start}-\DTMfetchmonth{fallPrecept1Start}-\numexpr\DTMfetchday{fallPrecept1Start}-1}
% End - 'don't change' section
...
\begin{center}
	\section[Fall Preceptorial 1]{Fall Preceptorial -- Sima Qian \\ \textit{Records of the Grand Historian (Shi Ji)}}
	 %\vspace{0.2cm}
	 (First Eight Weeks)
\end{center}
...
\begin{multicols}{2}
	...
	% TODO does 'photocopy' mean: in EC manual?
...
% date dancing, don't change - this gives us the day befor ethe first day of classes
%WHY?? well then all the function calls to print the session header below can be the same... confusing? a little, but hopefully void future errors in user-editable section below.
\DTMsavedate{lastPrintedClassDate}{\DTMfetchyear{fallPrecept2Start}-\DTMfetchmonth{fallPrecept2Start}-\numexpr\DTMfetchday{fallPrecept2Start}-1}
% End - 'don't change' section
...
\end{center}
% TODO: make section title repeate if new page is needed?
...
\begin{multicols}{2}
	...
	%TODO: quote chapter titles?
	%TODO: add Volume numbers? but some volumes have multiple boos, others don't
	\printPreceptHeader{lastPrintedClassDate}
	...
	%TODO: is 'discussion of whole of book 1' supposed to be a note on what the class will discuss?
	...
	% FIXME: add columnbreak? or otherwise see if the parenthetical is still broken over to 2nd column after edits.
...
% date dancing, don't change - this gives us the day befor ethe first day of classes
%WHY?? well then all the function calls to print the session header below can be the same... confusing? a little, but hopefully void future errors in user-editable section below.
\DTMsavedate{lastPrintedClassDate}{\DTMfetchyear{summerPreceptStart}-\DTMfetchmonth{summerPreceptStart}-\numexpr\DTMfetchday{summerPreceptStart}-1}
% End - 'don't change' section
...
\begin{center}
	\section[Summer Preceptorial]{Summer Preceptorial – \emph{The Tale of Genji}}
\end{center}



\textbf{Required Text:}
% TODO: why is there such a small line space between these two??
\newcommand{\summerPreceptReadingList}{%
	\begin{itemize}
		\item \emph{The Tale of Genji} by Murasaki Shikibu, translated by Edward G. Seidensticker (ISBN: 978-0679417385)
	\end{itemize}
}
